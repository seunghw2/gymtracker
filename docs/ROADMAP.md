...
> "무엇을 충족해야 하나"의 기준선과 갭 점검(audit)은 [REQUIREMENTS.md](REQUIREMENTS.md) 참고.
...
**A4. 리포트 기간별 "인격" (핵심 차별화)**
- [ ] 주간=전술 / 월간=성장 / 분기=전략 / 연간=회고로 **기간마다 역할·콘텐츠 분리**(지금은 날짜창만 다른 같은 통계). 위험알림·MVP·성장률·1RM변화·월간점수·정체원인·미래예측 신규 계산 + 기간별 AI 프롬프트. 상세 명세: [REPORT_REDESIGN.md](REPORT_REDESIGN.md).
