...
**운동 (`app/(tabs)/workout.tsx`)**: 세션 시작/종료(경과 타이머), 단계별 운동 추가(부위→장비→브랜드→종목), 커스텀 종목·브랜드 등록, 세트 입력(무게/횟수/완료 체크), 1RM 뱃지, 휴식 타이머, 스와이프 삭제, 이전 기록 자동 채움, 세션 히스토리 + 상세 모달
...
**A1. 통계 "볼륨" 탭 구현** (`stats.tsx:152` 현재 "준비 중")
- [ ] 백엔드: 기간별 볼륨(Σ 무게×횟수) 집계 API (`/api/v1/stats/volume?from=&to=`)
- [ ] 종목별/부위별 볼륨 분해 옵션
- [ ] 프론트: 볼륨 추이 차트 + 주/월 토글
- [ ] 빈 데이터 처리
