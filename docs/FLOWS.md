...
### 인터랙션 상태 (실제 캡처)
| 정렬 시트 + 편집모드 | 편집 1열 + 정렬 적용(드래그 잠김) |
|:--:|:--:|
| ![](screenshots/i1-sort-sheet.png) | ![](screenshots/i2-edit-mode.png) |

- **정렬 ⌄ → 시트**: 담은 순서 / 최근순 / 무게순 / 이름순 / 부위순. 선택 시 ✓ + 그리드 재정렬.
- **편집 → 모드 전환**: 1열 리스트로 바뀌며 카드에 **≡(드래그)·✕(삭제)** 노출, 버튼이 `완료`로. (위 우측: 정렬이 **담은 순서가 아니면 ≡ 드래그가 잠기고 ✕만** 노출 — 스펙대로.)
