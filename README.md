...
하단 탭은 **브리핑 · 기록 · 종목 · 리포트 · Chat** 5개. 운동은 어디서나 **슬라이드업 모달**로 열리고(브리핑의 "운동 시작" 또는 진행 중 배너), 설정은 브리핑 우측 상단 톱니로 진입한다.

| 브리핑(홈) | 기록 | 종목 | 리포트 | Chat |
|:--:|:--:|:--:|:--:|:--:|
| AI 브리핑 + 운동 시작 | 타임라인·캘린더 | 종목 watchlist + 종목별 리포트 | 기간별 AI 리포트 | AI 코치 대화 |
...
- **AI 코치 채팅(Chat 탭)**: 세션 기반 대화 — 정체/리포트/직접 질문이 **출처별 1세션**으로 묶여 누적·재방문(중복 방지), 클라 청크 스트리밍. 리포트 코치·정체 알림·종목 리포트의 "대화로 풀기"가 같은 키면 같은 대화로 이어짐
...
### 📈 종목별 리포트 (종목 탭)
- **종목 watchlist 허브**: 증권 앱처럼 ★보유(즐겨찾기)/관심 섹션 — 행마다 종목·부위 + 미니 스파크라인 + 현재 추정 1RM + 등락칩(상승/신기록 ▲그린·정체 ▬오렌지), 정렬(1RM/정체/최근)·검색
- **종목 리포트 화면**: 큰 숫자 헤더(현재 1RM·★PR/정체 배지·진단 한 줄) + **지표 토글 차트** — [1RM][최대무게]는 라인(1RM엔 PR★ 마커·정체 구간 음영, 기간칩 3M/6M/1Y/전체), [볼륨][빈도]는 주간 막대(이번 주 감소 강조)
- **종목 코치**: 진단/처방/동기 3줄 + "대화로 풀기"로 AI 코치 채팅 연결
- 즐겨찾기는 리포트 '주력 핀'과 공유. 1RM 성장·볼륨 추이·근육군 빈도·체중 그래프 등 추세 분석은 리포트/종목 화면에서 제공
...
| **차트** | react-native-chart-kit · react-native-svg(종목 리포트 라인/막대) |
...
│   ├── (auth)/                # 로그인 / 회원가입
│   ├── (tabs)/                # 브리핑(index)·기록(calendar)·종목(exercises)·Chat(chat)·설정(settings) + 합성 리포트 탭
│   │   ├── index.tsx          # 브리핑 홈 (AI 요약 + 운동 시작)
│   │   └── exercises.tsx      # 종목 watchlist 허브
│   ├── exercise/[name].tsx    # 종목별 리포트(헤더·지표 토글 차트·코치)
│   ├── chat/[conversationId].tsx  # AI 코치 대화 상세
│   └── ai/                    # AI 애널리스트
